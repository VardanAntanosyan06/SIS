...
const changeSubTaskStatus = async (req, res) => {
  try {
    ...
    const item = await SubTask_per_User.findOne({
      where: { subTaskId, userId: user.id },
    });
    const thisSubtask = await SubTaskModel.findOne({
      where: { id: subTaskId },
    });
    const thisTask = await Task_per_User.findOne({
      where: { taskId: mySubTask.taskId, userId: user.id },
    });
    ...
    if (item) {
      ...
      let completedSubTasks = myTask.filter(
        (e) => e.SubTask_per_Users[0].status === true
      );
      let taskStatus = "";
      if (completedSubTasks.length > 0) {
        if (new Date() > thisTask.deadline) {
          ...
          if (
            completedSubTasks.length === myTask.length &&
            thisTask.status !== "Completed"
          ) {
            const taskPoint = await TimeTaskModel.findOne({
              where: { task_id: mySubTask.taskId },
            });
