  ...
  try {
    ...
    if (user) {
      ...
      if(doneTasks.length>(LateDoneTasks.length+overdueTasks.length)){
        successMesange = await SuccessMessages.findOne({where:{status:"Excellent"},exclude:['id']})
      }else if(doneTasks.length===(LateDoneTasks.length+overdueTasks.length)){
        successMesange = await SuccessMessages.findOne({where:{status:"Good"},exclude:['id']})
        }else if(doneTasks.length<(LateDoneTasks.length+overdueTasks.length)){
        successMesange = await SuccessMessages.findOne({where:{status:"Unsatisfactory"},exclude:['id']})
      }
