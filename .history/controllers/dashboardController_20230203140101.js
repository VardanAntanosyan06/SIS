...
const dashboard = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const myTasks = await TaskModel.findAll({where:{userId:user.id}})
      const completedTask = await Task_per_User.findAll({
        where: { userId: user.id,status:"Completed" }
      });
      const completed = completedTask.length;
      console.log(myTasks.length,completed);
      const overallProgress = Math.round(completed*100/myTasks.length)
