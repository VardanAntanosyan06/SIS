...
const dashboard = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const safetyPointsExtra = myUni.sefetyPointMin; 
      let myPoints = 0;
      myTasks.forEach(el => {
        myPoints += el.point 
      });
      ...
      const extraculicular = Math.round(myPoints*100/safetyPointsExtra)

      //  const progress = Math.round(completed*totalPoints/100)
      return res.json({ TrainingDays,totalPoints,completed,extraculicular,myPoints,RandomGreetingMessages});
    } else {
