  ...
  try {
    ...
    if (user) {
      ...
      if(doneTasks.length>(LateDoneTasks.length+overdueTasks.length)){
        ...
        successMesange = "h our cutting-edge technology, you are on your way to securing a spot at your dream university soon. We hope to see you rise to an Excellent level soon. Keep up the good work. Sky's the limit, so Prepare Different.
        "
      }else if(doneTasks.length<(LateDoneTasks.length+overdueTasks.length)){
        successMesange = "c"
      }
