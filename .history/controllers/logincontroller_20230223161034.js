...
const isLogined = async (req,res)=>{
  try {
    ...
    const firstEmail = await UserEmails.findOne({where:{userId:user.id,role:"First"}})
    ...
    const SecondaryEmail = await UserEmails.findOne({where:{userId:user.id,role:"Secondary"}})
