...
const dashboard = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const overAllProgressInProgress = Math.round(myTasks.length/inProgressTasks.length*100*10)/10

      return res.json({ TrainingDays,totalPoints,completed,extraculicular,myPoints,RandomGreetingMessages,progressWithPercent,overAllProgressDone,ov});
    } else {
